...
import { toggleWishlist, renderWishlist, updateWishlistBadge, isWishlisted } from "./wishlist.js";
...
function setupEvents() {
  ...
  $("mobileSearchBtn")?.addEventListener("click", () => {
    const mobileValue = $("mobileSearchInput")?.value || "";
    if ($("searchInput")) $("searchInput").value = mobileValue;
    renderAll();
  });
  ...
  $("modalBuyNowBtn")?.addEventListener("click", () => {
    ...
    openOverlay("cartOverlay");
    showToast("Added. Continue checkout from cart");
  });
  ...
  $("signupForm")?.addEventListener("submit", (e) => {
    ...
    const name = $("signupName")?.value.trim() || "Customer";

    if (!email) {
      setAuthMessage("Enter email", true);
      return;
    }
  ...
  $("accountBtn")?.addEventListener("click", () => {
    showToast("Account frontend ready. Backend later.");
  });
  ...
  return `
    <article class="product-card" data-product-id="${product.id}">
      <div class="product-card-img-wrap">
        <img class="product-card-img" src="${product.images[0]}" alt="${product.name}" onerror="this.src='logo.png'">
      ...
      <div class="product-card-body">
        ...
        <div class="card-rating">★★★★★ <span class="rev-count">${product.reviews.length} reviews</span></div>
        <div class="product-card-actions">
          <button class="view-product-btn" type="button" data-view-id="${product.id}">VIEW</button>
          <button class="quick-wishlist-btn" type="button" data-wish-id="${product.id}">${wished ? "♥" : "♡"}</button>
        </div>
      </div>
    </article>
  `;
...
function renderProductScroller(containerId, list) {
  ...
  if (!container) return;
  container.innerHTML = list.length ? list.map(productCard).join("") : `<div class="empty-state">No products found</div>`;
...
function openProductDetail(productId) {
  ...
  $("modalThumbs").innerHTML = product.images.map((img, index) => `
    <img src="${img}" class="${index === 0 ? "active" : ""}" alt="${product.name} thumbnail" onerror="this.src='logo.png'">
  `).join("");
  ...
  $("modalSizeOptions").innerHTML = product.sizes.map((size) => `
    <button class="option-btn size-option" type="button" data-size="${size}">${size}</button>
  `).join("");
  ...
  $("modalColorOptions").innerHTML = product.colors.map((color) => `
    <button class="option-btn color-option ${color === selectedColor ? "active" : ""}" type="button" data-color="${color}">${color}</button>
  `).join("");
...
function renderAdmin() {
  ...
  $("statRevenue").textContent = `₹${state.orders.reduce((sum, order) => sum + order.total, 0)}`;
